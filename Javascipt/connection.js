...
function saveUser(username, password, phone, address){
  ...
  users.push({ username, password, phone, address, joined: Date.now() });
...
for(let i=0;i<users.length;i++)
{
    if(users[i].username===exUser&& users[i].password===exPass)
    {
            return true;
